...
def get_args_parser():
    ...
    # ++++++++++++ 新增：Look Twice (Soft-Seed Growing) 测试参数 ++++++++++++
    ...
    parser.add_argument('--lt_steps', default=3, type=int,
                        help='Iterations for Soft-Seed growing. Should match training.')
    ...
    parser.add_argument('--f1_mode', default='origin', type=str,
                        choices=['origin', 'double'],
                        help='origin: normal F1, double: Permute F1-score')
...
def main(args, model_args):
    ...
    # ]
    print(f"Using PixelF1 mode: {args.f1_mode}")
    evaluator_list = [
        PixelF1(threshold=0.5, mode=args.f1_mode),
        # ImageF1(threshold=0.5)
    ]
